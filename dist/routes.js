...
import { connectedDevices, devicesByTerminalId, receivedPackets, sendCommandToDevice, getServerStats, getDeviceByTerminalId, pendingRegistrations, queueCommand, } from "./tcpServer.js";
import { buildSealCommand, buildEnableFingerprintRegister, buildWriteICCard, buildEnableAutoCardBinding, buildSetCardTriggerMode } from "./hhd-protocol.js";
const router = Router();
...
router.get("/devices/:deviceId/packets", (req, res) => {
    ...
    const packets = receivedPackets.filter((p) => p.deviceId === deviceId || p.terminalId === deviceId);
...
router.post("/devices/:terminalId/seal", (req, res) => {
    try {
        ...
        const { seal, operatorName } = req.body;
        if (seal === undefined) {
...
router.post("/devices/:terminalId/fingerprint/autobinding", (req, res) => {
    try {
        const { terminalId } = req.params;
        const { minutes, userName } = req.body;
        if (!userName) {
            return res.status(400).json({ ok: false, message: "Debes enviar userName" });
        }
...
router.post("/devices/:terminalId/nfc/write", (req, res) => {
    try {
        const { terminalId } = req.params;
        const { cardId, blockNumber, address, userName } = req.body;
        if (!cardId) {
