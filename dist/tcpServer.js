...
export const devicesByTerminalId = new Map();
export const receivedPackets = [];
...
function normalizeRaw(buffer) {
    const raw = buffer.toString("utf8");
    return raw.replace(/[^\x20-\x7E\r\n\t]/g, ".");
}
export function getDeviceByTerminalId(terminalId) {
    return devicesByTerminalId.get(terminalId.toUpperCase());
}
// ✅ Encolar comando para cuando el candado se conecte
export function queueCommand(terminalId, command) {
    ...
    const server = net.createServer((socket) => {
        ...
        socket.on("data", (data) => {
            ...
            if (parsed) {
                ...
                if (receivedPackets.length > 1000)
                    receivedPackets.pop();
                // Reporte de posición
                ...
                const response = buildResponse8001(parsed.terminalId, parsed.serialNumber, parsed.msgId);
...
export function sendCommandToDevice(deviceId, command, mode = "text") {
    const device = connectedDevices.get(deviceId);
    if (!device) {
        throw new Error("El dispositivo no está conectado");
    }
    const buffer = mode === "hex"
